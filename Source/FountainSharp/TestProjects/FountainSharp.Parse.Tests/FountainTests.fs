...
TIM, is smiling broadly." |> Fountain.Parse
   doc.Blocks
   |> should equal [Action (false, [Literal "Natalie looks around at the group, TIM, ROGER, NATE, and VEEK."; HardLineBreak; HardLineBreak; Literal "TIM, is smiling broadly."])]
...
let ``Parenthetical `` () =
   ...
   doc.Blocks
   |> should equal [Character (false, [Literal "LINDSEY"]); Parenthetical [Literal "quietly"]];

[<Test>]
let ``Parenthetical - After Dialogue`` () =
   let doc = "LINDSEY\r\n(quietly)\r\nHello, friend.\r\n(loudly)\r\nFriendo!" |> Fountain.Parse
   doc.Blocks
   |> should equal [Character (false, [Literal "LINDSEY"]); Parenthetical [Literal "quietly"]; Dialogue [Literal "Hello, friend."]; Parenthetical [Literal "loudly"]; Dialogue [Literal "Friendo!"]];
...
let ``Dialogue - Normal`` () =
   ...
   doc.Blocks
   |> should equal [Character (false, [Literal "LINDSEY"]); Dialogue [Literal "Hello, friend."]]

[<Test>]
let ``Dialogue - After Parenthetical`` () =
   let doc = "LINDSEY\r\n(quietly)\r\nHello, friend." |> Fountain.Parse
   doc.Blocks
   |> should equal [Character (false, [Literal "LINDSEY"]); Parenthetical [Literal "quietly"]; Dialogue [Literal "Hello, friend."]]
...
let ``Transition - normal`` () =
   ...
   doc.Blocks
   |> should equal [Transition (false, [Literal "CUT TO:"])]

[<Test>]
let ``Transition - forced`` () =
   let doc = "> Burn to White." |> Fountain.Parse
   doc.Blocks
   |> should equal [Transition (true, [Literal "Burn to White."])]
...
let ``Notes - Inline`` () =
   ...
   |> should equal [Action (false, [Literal "fails anyway, it drops the note right now."])]
...
let ``Emphasis - Bold`` () =
   ...
   doc.Blocks
   |> should equal [Action (false, [Strong [Literal "This is bold Text"]])]

[<Test>]
let ``Emphasis - Italics`` () =
   let doc = "*This is italic Text*" |> Fountain.Parse
   doc.Blocks
   |> should equal [Action (false, [Italic [Literal "This is italic Text"]])]

[<Test>]
let ``Emphasis - Bold Italic`` () =
   let doc = "***This is bold Text***" |> Fountain.Parse
   doc.Blocks
   |> should equal [Action (false, [Strong [Italic [Literal "This is bold Text"]]])]
...
let ``Emphasis - with escapes`` () =
   ...
   doc.Blocks
   |> should equal [Action (false, [Literal "Steel enters the code on the keypad: "; Strong [Literal "*9765*"]])]
